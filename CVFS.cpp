...
void man(char *name)
{
    ...
    {
        printf("Description:Used t list all infoemation of files\n");
        printf("Usage:ls\n");
    }
    else if(strcmp(name,"stat")==0)
    {
        printf("Description:Used to diaplay information of the file\n");
...
void DisplayHelp()
{
    ...
    printf("truncate: To Removeall data from file \n");
...
int CreateFile(char *name,int permission)
{
    ...
    if((name==NULL) ||(premission==0)||(permission>3))
    {
        retuen -1;
    }
...
int OpenFile(char *name,int mode)
{
    ...
    UFDTArr[i].ptrfiletable=(PFILENAME)malloc(sizeof(FILETABLE));
    if(UFDTArr[i].ptrfiletable==NULL)
    {
        return -1;
    }
...
int LseekFile(int fd , int size , int from)
{
    ...
    if((UFDTArr[fd].ptrfilename->mode==READ) || (UFTDArr[fd].ptrfiletable->mode==READ+WRITE))
    {
        if(from==CURRENT)
        {
    ...
    else if (UFDTArr[fd].ptrfiletable->mode==WRITE)
    {
        if(from==CURRENT)
        {
            if(((UFDTArr[fd].ptrfiletable->rwriteoffset)+ size) > MAXFILESIZE)   return -1;
    ...
    while(1)
    {
        ...
        else if(count==2)
        {
            ...
            else if(strcmp(command[0],"close")==0)
            {
                ...
                if(ret==-1)
                    printf("ERROR: There is nos uch file \n");
                continue;
            }
            ...
            else if (strcmp(command[0],"write")==0)
            {
                ...
                if(ret==0)
                {
                   printf("ERROR: Incorrect parameters\n");
                    ...
                    if(ret==-2)
                        printf("Error :there is no sufficiemt memory to write \n");
                    if(ret==-3)
                        printf("Error: It is not a regular");
                }
                eles if (strcmp(command[0],"truncate")==0)
                {
            ...
            else if(count==3)
            {
                ...
                else if(strcmp(command[0],"read")==0)
                {
                    ...
                    if(ret==-4)
                        printf("Error:It si not a regular file\n");
                    if(ret >0)
                    {
                        write(2,ptr,ret);
                    }
